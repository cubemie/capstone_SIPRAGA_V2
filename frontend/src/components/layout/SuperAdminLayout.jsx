...
const NAV = [
  { to: '/superadmin/dashboard', icon: '🏠', label: 'Kelola Akun RT/RW' },
  { to: '/superadmin/template', icon: '📁', label: 'Kelola Template' },
  { to: '/superadmin/surat-manual', icon: '📝', label: 'Permintaan Manual' },
];
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="h-16 bg-[#0F2D5C] flex items-center justify-between px-4 md:px-6 flex-shrink-0 z-30 relative">
        <div className="flex items-center gap-3">
          <button className="md:hidden text-white p-1" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Buka menu">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </button>
          <span className="text-white font-bold text-lg tracking-tight">🏛️ SIPRAGA</span>
          <span className="hidden md:inline text-white/50 text-sm">Panel Superadmin</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2">
            <span className="text-white/80 text-sm">{user?.username}</span>
            <span className="bg-purple-500 text-white text-xs px-2 py-0.5 rounded-full font-medium">Superadmin</span>
          </div>
          <button onClick={handleLogout} className="text-white/70 hover:text-white text-sm border border-white/30 hover:border-white/60 px-3 py-1 rounded transition-colors">
            Keluar
          </button>
        </div>
      </header>
