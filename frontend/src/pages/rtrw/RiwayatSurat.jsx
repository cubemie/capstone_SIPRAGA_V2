      ...
      <div className="bg-white border border-neutral-100 rounded-lg shadow-sm">
        {loading ? (
          <div className="p-5 space-y-3">
            {[...Array(4)].map((_, i) => <div key={i} className="animate-pulse flex h-12 bg-neutral-50 rounded" />)}
          </div>
        ) : history.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-neutral-50 border-b border-neutral-100 text-gray-500 text-xs uppercase font-semibold">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {history.map(h => (
                  <tr key={h.id} className="hover:bg-primary-light/10 transition-colors">
                    ...
                    <td className="px-5 py-4 text-sm font-semibold text-neutral-900">{h.nama_warga || h.warga?.nama}</td>
